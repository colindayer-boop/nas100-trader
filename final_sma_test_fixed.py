...
def perf(series, label):
    ...
    years = (ret_df.index[-1] - ret_df.index[0]).days / 365.25. = (1 + total) ** (1 / years) - 1 if years > 0 else np.nan
    vol_ann = sr.std() * np.sqrt(252)
    sharpe = sr.mean() / sr.std() * np.sqrt(252) if sr.std() != 0 else np.nan
    dd = (cum / cum.cummax() - 1).min()
    print(f"{label:12} | CAGR: {c:.6%} | Vol: {vol_an:>5.2%} | Sharpe: {sharpe:>5.2f} | MaxDD: {dd:>6.2%}")
