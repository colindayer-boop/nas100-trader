...
def vol_target_weights(ret_df,target_vol=0.12,lookback=63,max_leverage=2.0):
    ...
    return weights
def walk_forward_debug(ret_df,n_splits=5,purge_days=5):
    n=len(ret_df)
    split_size=n//n_splits
    print(f"n={n}, split_size={split_size}")
    equity=[1.0]
    ...
    for i in range(n_splits):
        ...
        print(f"Iter {i}: train_end={train_end}, test_start={test_start}, test_end={test_end}")
        if test_start>=n:
            print("  break")
            break
        ...
        print(f"  port_ret head:\n{port_ret.head()}")
        if (port_ret!=0).any():
            print("  Non-zero returns found!")
        else:
            print("  All zero returns.")
        daily_growth=1+port_ret
        equity.extend((equity[-1]*daily_growth).tolist())
        dates.extend(test.index.tolist())
        print(f"  equity last few: {equity[-5:]}")
    return pd.Series(equity,index=pd.to_datetime(dates))
eq=walk_forward_debug(ret)
print("Final equity tail:",eq.tail())