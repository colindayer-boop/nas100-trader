...
port_vt = (weighted * ret_df).sum(axis=1) if USE_VOL_TARGET else port_eq
...
def perf(series, label):
    ...
    print(f"{label:12} | CAGR: {cagar:>6.2%} | Vol: {vol_an:>5.2%} | Sharpe: {sharpe:>5.2f} | MaxDD: {dd:>6.2%}")
