...
def get_regime():
    ...
    try:
        ...
        if vix.empty or len(vix) < 21:
            logger.warning("get_regime: VIX data empty/insufficient, defaulting to low-VIX regime")
            vix_ma21 = 15.0
        else:
            vix_ma21 = float(vix.rolling(21).mean().iloc[-1])
    except Exception as e:
        logger.warning(f"get_regime: VIX download failed: {e}, defaulting to low-VIX regime")
        vix_ma21 = 15.0
    # SPY
    try:
        ...
        if spy.empty or len(spy) < 200:
            logger.warning("get_regime: SPY data empty/insufficient, defaulting to bull")
            spy_bull = True
        else:
            spy_bull = bool(spy.ewm(span=50,adjust=False).mean().iloc[-1] >
                            spy.ewm(span=200,adjust=False).mean().iloc[-1])
    except Exception as e:
        logger.warning(f"get_regime: SPY download failed: {e}, defaulting to bull")
        spy_bull = True
    # QQQ
    try:
        ...
        if qqq.empty or len(qqq) < 200:
            logger.warning("get_regime: QQQ data empty/insufficient, defaulting to bull")
            qqq_bear200 = False
        else:
...
def get_gex_levels(broker, symbol="QQQ"):
    ...
    try:
        ...
        # Explicit guard: yfinance can return None/NaN for delisted or halted symbols.
        # The outer try/except already returns (None,None,None,None) on any exception,
        # but an explicit check here logs the specific failure for diagnostics.
        import math as _math
        if spot is None or _math.isnan(spot) or spot <= 0:
            logger.warning(f"get_gex_levels: invalid spot price for {symbol} (spot={spot}), returning None")
...
def run_s1(broker, equity, open_syms, vix_ma21, spy_bull, vix_mult):
    ...
    if signal_cond[recent.index].any():
        ...
        broker.place_order_safe("QQQ", shares, "buy", "S1",
                                ...
                                signal_price=price, decision_ts=now_et().isoformat())
    else:
...
def run_s3(broker, equity, open_syms, vix_mult):
    ...
    for sym in SYMBOLS:
        ...
        try:
            ...
            if len(d) < 70:
                logger.warning(f"S3 {sym}: only {len(d)} bars, skip")
                print(f"  {sym}: insufficient data ({len(d)} bars) - skip")
                continue
        except Exception as e:
            logger.warning(f"S3 {sym}: yfinance download failed: {e}")
            print(f"  {sym}: data fetch failed - skip")
            continue
...
def run_s5(broker, equity, open_syms, vix_ma21, spy_bull, qqq_bear200):
    ...
    if price > orb_high and spy_bull and vol_ok:
        ...
        broker.place_order_safe("QQQ", shares, "buy", "S5",
                                ...
                                signal_price=price, decision_ts=now_et().isoformat(), signal_bar_ts=cur.index[-1].isoformat())
...
def run_btc(broker, equity, open_syms):
    ...
    if data is None or data.empty:
        logger.warning("BTC: no hourly bar data, skipping")
        print("  No data for BTC - skip"); return
    ...
    if daily is None or daily.empty:
        logger.warning("BTC: no daily bar data for trend, skipping entry")
        print("  No daily data for trend - skip entry"); return
    dc = daily["Close"]
...
def run_overnight(broker, equity, open_syms):
    ...
    if not held and in_close_window and wd in (0, 1):
        try:
            _ovn_bars = broker.get_bars(OVN_SYMBOL, "1Hour", 5)
            if _ovn_bars is None or _ovn_bars.empty:
                logger.warning(f"OVN: no bar data for {OVN_SYMBOL}, skipping")
                print(f"  No data for {OVN_SYMBOL} - skip"); return
            price = float(_ovn_bars["Close"].iloc[-1])
        except Exception as e:
            logger.warning(f"OVN: bar data fetch failed: {e}")
            print("  Data fetch failed - skip"); return
...
def run_btc_trend(broker, equity, open_syms):
    ...
    daily = broker.get_bars("BTC", "1Day", 250)
    if daily is None or daily.empty:
        logger.warning("BTC-TREND: no daily bar data, skipping")
